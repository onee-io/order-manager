...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <form class="form-horizontal" method="post" action="/role-manager/edit-role/2/<?= $roleInfo['id'] ?>">
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <select id="role" name="role_type" class="form-control input-sm" required>
                        ...
                        <option value="1" <?= $roleInfo['role_type']==1?'selected':null ?>>发起者</option>
                        <option value="2" <?= $roleInfo['role_type']==2?'selected':null ?>>受理者</option>
                        <option value="3" <?= $roleInfo['role_type']==3?'selected':null ?>>管理员</option>
                    </select>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <input type="text" class="form-control input-sm" id="role_name" name="role_name" value="<?= $roleInfo['role_name'] ?>" placeholder="请输入角色名称" required>
                </div>
            </div>
            <div class="form-group">
                                ...
                                foreach ($classificationInfos as $classificationInfo) {
                                    ...
                                    if ($classificationIds) {
                                        foreach($classificationIds as $classificationId) {
                                            if ($classificationId == $classificationInfo['id']) {
                                                $isChecked = true;
                                            }
                                        }
                                    }
                                    if ($isChecked) {
                                        echo '<input checked id="'.$classificationInfo['classification_name'].'"type="checkbox" class="checkbox-inline" name="classification[]" style="margin-right:5px;" value="'.$classificationInfo['id'].'">';
